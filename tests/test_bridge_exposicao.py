...
a caixa-mae-aproximada cresceu 9->10 (cmt) e em 20/07 o dono a REMOVEU
(tupla volta a 9). Se mudar de novo, atualize _coletar() e a chamada de escrever() AQUI,
...
def _coletar(cfg, alvo):
    """Desempacota a tupla ATUAL de coletar() (9 elementos desde 20/07).
    ...
    cat, ven, ent, ped, pv, vm, hc, vc, val = bridge.coletar(cfg, True, alvo)
...
def test_only_exposicao_escreve_os_dois_csvs():
    with tempfile.TemporaryDirectory() as d:
        cfg = _cfg_demo(d)
        cat, ven, ent, ped, pv, vm, hc, vc, val = _coletar(cfg, "exposicao")
        rel = bridge.escrever(cfg, cat, ven, ent, ped, pv, vm, hc, vc,
                              alvo="exposicao", val=val)
...
def test_movimentos_nao_paga_a_query_de_exposicao():
    with tempfile.TemporaryDirectory() as d:
        cfg = _cfg_demo(d)
        _, ven, _, _, _, _, _, vc, _ = _coletar(cfg, "movimentos")
...
def test_demo_completo_nao_quebra():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        cat, ven, ent, ped, pv, vm, hc, vc, val = _coletar(cfg, "all")
        rel = bridge.escrever(cfg, cat, ven, ent, ped, pv, vm, hc, vc,
                              alvo="all", val=val)
        assert any("vendas_canal.csv" in r for r in rel)
