...
- **Entra na lista quando:** dias em aberto ≥ limiar (padrão **7**, configurável)
  **OU** previsão de entrega vencida.
...
"painel": {
  ...
  "detector_dashboard_url": "http://localhost:<porta-do-detector>/"
}
```
...
## 10. Investigações pré-implementação (no ponte, `inspect_schema.py`)

a. **Marcador de promoção relâmpago** no Solidcon (termos: promo/oferta/relampago/
   tipo). **Fallback aprovado:** se o ERP não distinguir tipo de promoção, usar toda
   promoção vigente do catálogo (`preco_promocao` + vigência) — validar com o dono
   um exemplo conhecido antes do 1º uso real.
b. **Fornecedor no pedido de compra** (coluna de ligação a `tbPessoa`) + telefone
   do fornecedor para o card de cobrança + confirmação do cálculo do **valor
   pendente** (`(qtPedidoItem - qtAtendida) × vlPedidoItem`? validar semântica
   de `vlPedidoItem` por volume × unidade, como no PEDIDOS_VENDA).
c. **Formato do round JSON** do detector-estoque (campos de prioridade/status).
...
- "Relâmpago" depende da investigação §10a; até lá o quadrante pode operar no
  fallback (toda promoção vigente).
