...
# CAIXA_MAE_TESTEMUNHAS: fontes alternativas de caixa-mae para os itens SEM
# QUANTIDADE_CAIXA no cadastro Neogrid (~708 no relatorio de exposicao).
# Pedido do dono (18/07): reduzir a lista sem presuncao ousada — preencher
# "caixa aproximada" SO quando fontes medidas concordam; o resto fica para
# ajuste manual.
#
# Uma linha por (produto, fonte), e cada fonte ja sai UNANIME (HAVING
# COUNT(DISTINCT)=1 — fonte que se contradiz nao vota). A projecao
# (catalogo_exposicao_csv) cruza os votos: todos iguais -> aproximada;
# qualquer discordancia -> fica sem (verificar manualmente).
#
# PRECISAO MEDIDA em 18/07 contra o gabarito de 3.872 itens COM caixa no
# cadastro (scripts/caixa-mae-valida-testemunhas.py):
#   pedido  98,8%  (tbPedidoItem.qtEmbalagem>1, compras 12m)
#   nota    98,4%  (tbNotaItem.qtEmbalagem>1, entradas 12m; =1 e fornecedor
#                   faturando em unidade — ausencia de opiniao)
#   nfe     96,8%  (tbProdutoPessoaComercialNFe.FatorConversaoUnitario, SO
#                   quando uComNFe e unidade de CAIXA — em KG o fator e
#                   conversao kg->bandeja, a armadilha da carne moida)
#   emb    100,0%  (tbEmbalagemSuperProduto qtEmbalagem>1 — e a origem da
#                   view Neogrid, quase nunca opina nos sem-caixa; fica como
#                   testemunha de veto)
# REPROVADAS (nunca usar): EAN do PDV qtVenda 51% (12 e valor-padrao em 12k
# produtos) e "C12" da descricao 19% (costuma ser o display, nao a caixa).
# So fator INTEIRO vale (12.0 sim, 12.5 nao — meia caixa nao existe).
CAIXA_MAE_TESTEMUNHAS = """
SELECT codigo, fonte, fator FROM (
    SELECT i.cdProduto AS codigo, 'nota' AS fonte,
    ...
    HAVING COUNT(DISTINCT i.qtEmbalagem) = 1
    UNION ALL
    SELECT i.cdProduto, 'pedido', CAST(MAX(i.qtEmbalagem) AS int)
    FROM dbo.tbPedidoItem i
    JOIN dbo.tbPedido p
      ON p.cdPedido = i.cdPedido AND p.cdPessoaFilial = i.cdPessoaFilial
    WHERE p.inEntrada = 1
    ...
    HAVING COUNT(DISTINCT i.qtEmbalagem) = 1
    UNION ALL
    SELECT n.cdProduto, 'nfe', CAST(MAX(n.FatorConversaoUnitario) AS int)
    FROM dbo.tbProdutoPessoaComercialNFe n
    WHERE n.FatorConversaoUnitario > 1
      AND n.FatorConversaoUnitario = FLOOR(n.FatorConversaoUnitario)
      AND n.cdProduto IS NOT NULL
      AND UPPER(LTRIM(RTRIM(n.uComNFe))) IN ('CX','CX1','CX2','CX3','FD','DP',
                                             'FRD','CAIXA','FARDO','DZ','PC',
                                             'PCT','SC')
    GROUP BY n.cdProduto
    HAVING COUNT(DISTINCT n.FatorConversaoUnitario) = 1
    UNION ALL
    SELECT p.cdProduto, 'emb', CAST(MAX(e.qtEmbalagem) AS int)
    ...
    HAVING COUNT(DISTINCT e.qtEmbalagem) = 1
) t
ORDER BY codigo, fonte
"""
